...
class CryptoComparePricesAPITests: XCTestCase {
    ...
    func testGetCoinPrices() async throws {
        let prices = try await pricesProvider.getCurrentPrices(coins: ["BTC", "ETH"], toFiat: "USD")
        XCTAssertEqual(prices["ETH"]??.value, 1796.09)
        XCTAssertEqual(prices["BTC"]??.value, 30231.69)
    }
    
    func testGetHistoricalPrices() async throws {
        let historicalPrices = try await pricesProvider.getHistoricalPrice(of: "BTC", fiat: "USD", period: .month)
